...
const Card = ({ data }) => {
    ...
    useEffect(() => {
        api.get(data?.url).then(res => console.log(res)).catch(err => console.error(err))
    }, [])

    return (
        <>
            <Link to={`/${data?.name}`}>
                <main style={{ backgroundColor: "rgba(4, 171, 4, 0.681)" }} className={`${s.card} ai__center`}>
                    <section className={`${s.info} flex column`}>
                        <h1>Bulbasaur</h1>
                        <div className={`${s.type} ai__center`}>
